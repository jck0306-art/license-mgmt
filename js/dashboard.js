...
export function renderCertDashboard(cert) {
  ...
  mainEl.innerHTML = `
    <div class="flex justify-between items-start mb-6">
      ...
      <button onclick="window.deleteCert('${cert.id}')" class="text-xs text-rose-400 hover:text-rose-300 border border-rose-500/30 px-3 py-1.5 rounded transition hover:bg-rose-500/10">자격증 삭제</button>
    </div>
    ...
    <div class="bg-slate-800 rounded-xl border border-slate-700 p-5 mb-6 shadow-lg">
      <div class="flex justify-between items-center mb-3">
        ...
        <button onclick="window.addSchedulePrompt('${cert.id}')" class="text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-2.5 py-1.5 rounded transition flex items-center gap-1 font-semibold">
        ...
        <table class="w-full text-left text-xs">
          ...
          <tbody class="divide-y divide-slate-700/60">
            ${cert.schedules.length === 0 ? `
              <tr><td colspan="5" class="p-4 text-center text-slate-500">등록된 시험 일정이 없습니다. 일정을 추가해보세요.</td></tr>
                ...
                <tr class="hover:bg-slate-900/40">
                  <td class="p-2.5 font-medium text-slate-200">
                    <div class="flex items-center gap-1.5">
                      <span>${sch.name}</span>
                      <button onclick="window.editScheduleNamePrompt('${cert.id}', '${sch.id}')" class="text-slate-500 hover:text-amber-300 text-xs p-1" title="회차명 수정">
                  ...
                  <td class="p-2.5 font-mono">
                    <div class="flex items-center gap-1.5">
                      <input type="date" value="${sch.date}" onchange="window.updateScheduleDate('${cert.id}', '${sch.id}', this.value)" class="bg-slate-900 border border-slate-700 rounded px-2 py-0.5 text-xs text-white focus:outline-none focus:border-indigo-500" />
                      <span class="text-[11px] px-1.5 py-0.5 rounded bg-slate-900 border border-slate-700 text-indigo-400 font-bold">${dday}</span>
                    </div>
                  </td>
                  <td class="p-2.5">
                    <select onchange="window.updateScheduleStatus('${cert.id}', '${sch.id}', 'applied', this.value)" class="bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 cursor-pointer">
                  ...
                  <td class="p-2.5">
                    <select onchange="window.updateScheduleStatus('${cert.id}', '${sch.id}', 'result', this.value)" class="bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 cursor-pointer">
                  ...
                  <td class="p-2.5 text-right">
                    <button onclick="window.deleteSchedule('${cert.id}', '${sch.id}')" class="text-slate-500 hover:text-rose-400 p-1" title="일정 삭제"><i class="fa-solid fa-trash"></i></button>
                  </td>
                </tr>
              `;
            }).join('')}
    ...
    <div class="bg-slate-800 rounded-xl border border-slate-700 p-4 mb-6 flex flex-wrap items-center justify-between gap-3 shadow-lg">
      ...
      <div class="flex items-center gap-2">
        <button onclick="window.setCertGoogleSheetUrlPrompt('${cert.id}')" class="text-xs bg-slate-700 hover:bg-slate-600 text-slate-200 px-3 py-2 rounded-lg transition font-medium">
    ...
    <div class="bg-slate-800 rounded-xl border border-slate-700 p-5 mb-6 shadow-lg">
      ...
      <div class="flex gap-2 mb-4">
        <input id="memo-input" type="text" placeholder="자격증과 관련된 메모를 입력하세요..." 
          ...
          onkeydown="if(event.key === 'Enter') window.addMemo('${cert.id}')" />
        <button onclick="window.addMemo('${cert.id}')" class="bg-indigo-600 hover:bg-indigo-500 text-white text-xs px-4 py-2 rounded-lg font-semibold transition flex items-center gap-1.5 whitespace-nowrap">
        ...
        ` : cert.memos.map(memo => `
          <div class="flex items-center justify-between p-3 rounded-lg bg-slate-900/90 border border-slate-700/80 hover:border-slate-600 transition group">
            <div class="flex items-start gap-2 flex-1 mr-3">
              ...
              <p class="text-sm text-slate-200 leading-relaxed break-all">${memo.content}</p>
            </div>
            <div class="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition">
              <button onclick="window.editMemoPrompt('${cert.id}', '${memo.id}')" class="text-slate-400 hover:text-amber-300 p-1.5 rounded hover:bg-slate-800 text-xs"><i class="fa-solid fa-pen"></i></button>
              <button onclick="window.deleteMemo('${cert.id}', '${memo.id}')" class="text-slate-400 hover:text-rose-400 p-1.5 rounded hover:bg-slate-800 text-xs"><i class="fa-solid fa-trash"></i></button>
            </div>
          </div>
        `).join('')}
      </div>
    ...
    <div class="bg-slate-800 rounded-xl border border-slate-700 p-5">
      <div class="flex justify-between items-center mb-4">
        ...
        <button onclick="window.addSubjectPrompt('${cert.id}')" class="text-xs text-indigo-400 hover:underline">+ 과목 추가</button>
      </div>
      <div class="space-y-3">
        ${cert.subjects.map(sub => `
          <div class="flex items-center justify-between p-3.5 rounded-lg bg-slate-900 border border-slate-700/60">
            <div class="flex items-center gap-3 flex-1 cursor-pointer" onclick="window.selectSubject('${cert.id}', '${sub.id}')">
              <span class="text-sm font-medium text-slate-200 hover:text-indigo-400 transition">${sub.name}</span>
              <span class="text-xs text-amber-400/80 ml-2 font-mono">암기카드 ${(sub.notes || []).length}개</span>
            </div>
            <div class="flex items-center gap-3">
              <button onclick="window.editSubjectNamePrompt('${cert.id}', '${sub.id}')" class="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 px-2.5 py-1.5 rounded transition" title="과목명 수정">
              ...
              <div class="flex items-center gap-1 text-xs bg-slate-800 px-2 py-1 rounded border border-slate-700">
                ...
                <button onclick="window.updateCount('${cert.id}', '${sub.id}', 1)" class="ml-1 px-1.5 py-0.5 bg-slate-700 hover:bg-slate-600 rounded text-white font-bold">+</button>
                <button onclick="window.updateCount('${cert.id}', '${sub.id}', -1)" class="px-1.5 py-0.5 bg-slate-700 hover:bg-slate-600 rounded text-white font-bold">-</button>
              </div>
              <button onclick="window.deleteSubject('${cert.id}', '${sub.id}')" class="text-slate-500 hover:text-rose-400 text-xs ml-1"><i class="fa-solid fa-trash"></i></button>
            </div>
          </div>
        `).join('')}
      </div>
...
export function addSchedulePrompt(certId, onRender) {
  const name = prompt('시험 명칭/회차를 입력하세요 (예: 2026년 2회차, 임시시험):', '2026년 추가 시험');
  if (!name) return;
