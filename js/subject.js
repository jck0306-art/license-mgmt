...
export function renderSubjectDetailPage(cert, subId) {
  ...
  mainEl.innerHTML = `
    <div class="mb-6 flex justify-between items-center">
      <div>
        <button onclick="window.selectCert('${cert.id}')" class="text-xs text-indigo-400 hover:underline flex items-center gap-1 mb-2">
          <i class="fa-solid fa-arrow-left"></i> ${cert.name} 종합 대시보드로 돌아가기
        </button>
        <div class="flex items-center gap-3">
          <h2 class="text-2xl font-bold text-white flex items-center gap-2">
            <i class="fa-solid fa-book-open text-amber-400"></i> ${sub.name}
          </h2>
          <button onclick="window.editSubjectNamePrompt('${cert.id}', '${sub.id}')" class="text-xs text-slate-400 hover:text-amber-300 border border-slate-700 px-2.5 py-1 rounded bg-slate-800 transition">
      ...
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-1 text-xs bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700">
          ...
          <span class="font-bold text-indigo-400 font-mono">${sub.count || 0}회</span>
    ...
    <div class="bg-slate-800 rounded-xl border border-slate-700 p-5 mb-6 shadow-lg">
      ...
      <div class="space-y-3">
        <input id="note-title-input" type="text" placeholder="암기 키워드 / 법조문 / 핵심 제목" 
        ...
        <textarea id="note-body-input" rows="3" placeholder="상세 암기 내용 (두문자, 요약 정리, 풀이 팁 등)" 
        ...
        <div class="flex justify-end">
          <button onclick="window.addSubjectNoteInline('${cert.id}', '${sub.id}')" class="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs px-5 py-2.5 rounded-lg transition flex items-center gap-1.5">
        ...
        <div class="p-4 rounded-xl bg-slate-800 border ${note.done ? 'border-emerald-500/40 bg-slate-800/50' : 'border-slate-700'} transition group shadow-md space-y-2">
          <div class="flex items-start justify-between gap-3">
            <label class="flex items-center gap-2.5 cursor-pointer select-none">
              <input type="checkbox" ${note.done ? 'checked' : ''} onchange="window.toggleNoteDoneInline('${cert.id}', '${sub.id}', '${note.id}')" class="w-4 h-4 rounded text-emerald-500 bg-slate-900 border-slate-600 focus:ring-0">
              <span class="font-bold text-sm ${note.done ? 'line-through text-slate-500' : 'text-amber-300'}">${note.title}</span>
            </label>
            <div class="flex items-center gap-1 opacity-80 group-hover:opacity-100">
              <button onclick="window.editSubjectNoteInline('${cert.id}', '${sub.id}', '${note.id}')" class="text-slate-400 hover:text-amber-300 p-1.5 rounded hover:bg-slate-700 text-xs" title="수정">
                <i class="fa-solid fa-pen"></i>
              </button>
              <button onclick="window.deleteSubjectNoteInline('${cert.id}', '${sub.id}', '${note.id}')" class="text-slate-400 hover:text-rose-400 p-1.5 rounded hover:bg-slate-700 text-xs" title="삭제">
                <i class="fa-solid fa-trash"></i>
              </button>
            </div>
          </div>
          <p class="text-xs text-slate-300 whitespace-pre-wrap pl-6 leading-relaxed ${note.done ? 'text-slate-500' : ''}">${note.body || ''}</p>
        </div>
      `).join('')}
    </div>
  `;
...
export function addSubjectNoteInline(certId, subId, onRender) {
  ...
  sub.notes.unshift({ id: 'n_' + Date.now(), title, body, done: false });
  titleInput.value = '';
  bodyInput.value = '';
  saveState();
  if (onRender) onRender();
